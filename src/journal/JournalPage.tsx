...
export function JournalPage({ repo, space, refresh }: PageProps) {
  return (
    <>
      <PageHeading
        title="给情绪，一个停靠的地方"
        description="不必组织好语言，也不必急着变好。从记录此刻开始。"
      />
      <div className="journal-layout">
        <JournalForm
          key={space}
          onSave={async (entry) => {
            await repo.saveJournal(space, entry);
            await refresh();
          }}
        />
        <aside className="journal-aside">
          <div className="aside-quote">
            ...
            <Leaf className="aside-leaf" size={80} strokeWidth={0.7} />
          </div>
          <div className="gentle-note">
            <span>一个小提醒</span>
            <p>
              先描述发生了什么，再试着命名你的感受。不评价，也不着急寻找原因。
            </p>
          </div>
