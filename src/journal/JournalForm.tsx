...
export function JournalForm({
  ...
  const [occurred, setOccurred] = useState(() =>
    datetimeInput(initial ? new Date(initial.occurredAt) : new Date()),
  );
  const [feeling, setFeeling] = useState<Score | undefined>(initial?.feeling),
    [emotion, setEmotion] = useState<Emotion[]>(initial?.emotions ?? []);
  const [strength, setStrength] = useState<Score | undefined>(
      initial?.intensity,
    ),
    [events, setEvents] = useState<EventTag[]>(initial?.tags ?? []);
