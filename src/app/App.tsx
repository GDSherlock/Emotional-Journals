...
function Workspace({
  ...
  async function switchSpace() {
    ...
    try {
      const target = (space === "personal" ? "demo" : "personal") as Space;
      if (target === "demo" && !prefs.demoInitialized)
        await repo.replace("demo", seedDemo(new Date()));
      const next = {
        ...prefs,
        space: target,
        demoInitialized: prefs.demoInitialized || target === "demo",
        rangeDays: target === "demo" ? (30 as const) : prefs.rangeDays,
      };
...
export function App() {
    ...
    openRepository(import.meta.env.BASE_URL)
      .then(async (value) => {
        ...
        for (const space of ["personal", "demo"] as const) {
          const snapshot = await value.read(space);
          for (const item of snapshot.care) {
            if (item.status === "running" || item.status === "paused")
              await value.saveCare(space, {
                ...item,
                status: "interrupted",
                after: undefined,
              });
        ...
        const pref = await value.readPreferences();
        if (active) {
          setRepo(value);
          setPrefs(pref);
        } else value.close();
  ...
  async function choose(space: Space) {
    ...
    setBusy(true);
    try {
      if (space === "demo" && !prefs.demoInitialized)
        await repo.replace("demo", seedDemo(new Date()));
      const next = {
        ...prefs,
        space,
        demoInitialized: prefs.demoInitialized || space === "demo",
        rangeDays: space === "demo" ? (30 as const) : prefs.rangeDays,
      };
      await repo.savePreferences(next);
      setPrefs(next);
    } catch (e) {
      ...
      <main className="welcome">
        ...
        <div className="welcome-choices">
          ...
          >
            ...
            <small>虚构数据 · 可自由探索</small>
          </button>
          <button
            aria-label="开始我的记录"
            disabled={busy}
            className="welcome-choice personal-choice"
          ...
          >
            ...
            <small>
              <LockKeyhole size={13} /> 仅保存在当前浏览器
            </small>
          </button>
        </div>
        <p className="welcome-note">这是日常自我关怀工具，不能替代专业帮助。</p>
      </main>
    );
