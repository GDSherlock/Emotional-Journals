...
test('valid backup round-trips personal diary and feedback',()=>{expect(decodeBackup(encodeBackup(snapshot,now),now)).toEqual(snapshot);});
test('rejects unknown version, duplicate IDs, orphan references and invalid records',()=>{
 for(const change of [{version:2},{journals:[journal(),journal()]},{care:[care({journalId:'missing'})]},{journals:[journal({feeling:6 as never})]},{journals:[journal({text:'字'.repeat(5001)})]},{care:[care({status:'interrupted',after:4})]},{care:[care({endedAt:'2026-09-19T00:00:00Z'})]}])expect(()=>decodeBackup(JSON.stringify({...backup,...change}),now)).toThrow();
...
test('enforces byte limit before parsing and rejects malformed timestamps',()=>{
 ...
 expect(()=>decodeBackup(JSON.stringify({...backup,exportedAt:'not-a-date'}),now)).toThrow();
