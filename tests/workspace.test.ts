...
test("late personal read cannot replace current demo state", async () => {
  ...
  repo.read = async (space) =>
    space === "personal"
      ? pending
      : { journals: [journal({ id: "demo" })], care: [] };
  const { result, rerender } = renderHook(
    ({ space }: { space: Space }) => useWorkspace(repo, space),
    { initialProps: { space: "personal" as Space } },
  );
  rerender({ space: "demo" });
  await waitFor(() =>
  ...
  await act(async () =>
    resolve({ journals: [journal({ id: "personal" })], care: [] }),
  );
  expect(result.current.snapshot.journals[0].id).toBe("demo");
});
